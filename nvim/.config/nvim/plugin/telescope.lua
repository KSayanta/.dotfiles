local telescope_plugins = {
  ...
  _G.gh('nvim-telescope/telescope-ui-select.nvim'),
}

if vim.fn.executable('make') == 1 then table.insert(telescope_plugins, _G.gh('nvim-telescope/telescope-fzf-native.nvim')) end
...
map('n', '<leader>sg', builtin.live_grep, { desc = '[S]earch with [G]rep' })
map('n', '<leader>sh', builtin.help_tags, { desc = '[S]earch [H]elp' })
map('n', '<leader>sk', builtin.keymaps, { desc = '[S]earch [K]eymaps' })
map('n', '<leader>sm', builtin.marks, { desc = '[S]earch [M]arks' })
map('n', '<leader>sr', builtin.resume, { desc = '[S]earch [R]esume' })
map('n', '<leader>ss', builtin.builtin, { desc = '[S]earch [S]elect Telescope' })
map('n', '<leader>st', builtin.colorscheme, { desc = '[S]earch [C]olorscheme' })
